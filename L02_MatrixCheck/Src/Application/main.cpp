...
void Application::Update()
{
	...
	//キャラ制御
	{
		...
		//if (GetAsyncKeyState('W')) { _moveVec.z = 2.0f; }
		//if (GetAsyncKeyState('A')) { _moveVec.x = -2.0f; }
		//if (GetAsyncKeyState('S')) { _moveVec.z = -2.0f; }
		//if (GetAsyncKeyState('D')) { _moveVec.x = 2.0f; }
		...
		////太陽
		//
		//Math::Vector3 m_pos = m_HamuWorld.Translation();

		//Math::Matrix transMat = Math::Matrix::CreateTranslation(1, 1, 1);

		////Math::Matrix worldMat = transMat;
		//m_HamuWorld = Math::Matrix::CreateTranslation(m_pos);
	...
	//	Math::Matrix _mTrans = Math::Matrix::CreateTranslation(0.0f, 5.0f, -5.0f);
	//	//Math::Matrix _mTrans = Math::Matrix::CreateTranslation(0.0f, 5.0f, -5.0f);

	//	//Math::Matrix _mRot = Math::Matrix::CreateRotationX(DirectX::XMConvertToRadians(30)); //奥行調整
	//	
	//	//行列の合成　基本系		S　　*		R１　　*	T　　*		R２
	//	Math::Matrix  _mWorld = (_mScale * _mRotationX * _mTrans * _mRotationY);
	//	//Math::Matrix  _mWorld = _mScale * _mRotationX * _mTrans * _mRotationY;
	//	
	//	//行列の合成　キャラがカメラに追尾する行列		親子関係
...
//	}
	...
	m_spPoly->SetMaterial("Asset/Planets/sun.gltf");
	//m_spPoly->SetPivot(KdSquarePolygon::PivotType::Center_Bottom);

	//地球
	m_earth = std::make_shared<KdModelData>();
	m_earth->Load("Asset/Planets/earth.gltf");
	//太陽
